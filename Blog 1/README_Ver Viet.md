...
Các tiểu mục tiếp theo sẽ lần lượt giới thiệu qua các phương pháp như Trực quan hoá dữ liệu, Kiểm định thống kê, Học máy truyền thống, Học sâu hiện đại và phương pháp kết hợp cả các thuật toán truyền thống lẫn Học sâu.

## **4.1. Trực quan hóa dữ liệu (Visualization)**

Sử dụng hình ảnh trực quan cho việc phát hiện bất thường là phương pháp then chốt khi cần nhận diện nhanh các mẫu dữ liệu lạ, giải thích lý do tại sao một điểm dữ liệu bị coi là bất thường, hoặc giám sát các hệ thống phức tạp trong thời gian thực.
...
- **Nhận diện xu hướng và tính chu kỳ:** Biểu đồ đường (Line charts) cực kỳ hữu ích để phát hiện các thay đổi đột ngột hoặc sự gián đoạn trong dữ liệu chuỗi thời gian (time-series).
...
### **4.1.4. Truyền thông cho đối tượng không chuyên kỹ thuật**

Hình ảnh trực quan là cách tốt nhất để trình bày các phát hiện phức tạp cho các bên liên quan (stakeholders).
- **Báo cáo kinh doanh:** Biểu đồ giúp người quản lý thấy rõ sự sụt giảm doanh số bất thường mà không cần đọc qua hàng nghìn dòng bảng tính.
...
### **4.2.2. Cần kiểm soát tỷ lệ "Báo động giả" (False Positives)**

Kiểm định thống kê cho phép bạn thiết lập một ngưỡng tin cậy (như p-value < 0.05).

- **Độ tin cậy cao:** Trong các lĩnh vực như y tế hoặc tài chính, việc định lượng xác suất một điểm là bất thường giúp giảm thiểu rủi ro đưa ra quyết định sai lầm dựa trên dữ liệu nhiễu.
- **Xác thực mô hình:** Sử dụng để đánh giá xem sự khác biệt giữa dữ liệu thực tế và dự báo của mô hình có ý nghĩa thống kê hay không.

### **4.2.3. Yêu cầu tính minh bạch và giải thích được (Explainability)**

Khác với các mô hình "hộp đen" của AI, kiểm định thống kê cung cấp lý do rõ ràng.
...
Các phương pháp Học sâu rất mạnh nhưng cực kỳ ngốn phần cứng và dữ liệu.
...
Nếu dữ liệu của bạn có hình dạng kỳ quái, đa đỉnh (multi-modal) hoặc không có quy luật thống kê rõ ràng. Thuật toán dựa trên khoảng cách/mật độ như K-Nearest Neighbors (KNN) hoặc DBSCAN sẽ dựa vào khoảng cách vật lý giữa các điểm để xác định sự bất thường thay vì dựa vào các giả định toán học khắt khe.
...
Sau đây là một số cách tiếp cận mô hình học sâu.

### **4.4.1. Cách tiếp cận dựa trên khả năng tái tạo của mô hình (Reconstruction-based Approach)**

![Tiếp cận (a)](https://arxiv.org/html/2503.13195v1/extracted/6287086/Figures/three.jpg)
...
#### **4.4.1.1. Các mô hình GAN**

![GAN](https://arxiv.org/html/2503.13195v1/extracted/6287086/Figures/GAN.jpg)

Các mô hình GAN có khả năng tạo ra dữ liệu giống với dữ liệu trong tập huấn luyện. Các mô hình này có hai thành phần chính là bộ sinh (generator) và bộ phân biệt (discriminator), đều là các mạng nơ-ron. Bộ sinh $G$ lấy vào một vectơ nhiễu ngẫu nhiên $z$, và tạo ra $G(z)$. Bộ phân biệt $D$ lấy vào dữ liệu bình thường hoặc $G(z)$ và phải phân biệt được đâu là dữ liệu bình thường hoặc dữ liệu do bộ sinh tạo ra. Hai thành phần này được huấn luyện đồng thời với nhau; mục đích của bộ sinh là tạo ra dữ liệu gần với dữ liệu bình thường nhất có thể để lừa bộ phân biệt $D$, trong khi đó bộ phân biệt cố gắng để cải thiện phán đoán của mình.

Sau khi huấn luyện được mô hình, dữ liệu được đưa vào cả bộ sinh và bộ phân biệt. Bộ sinh tái tạo ra $G(z)$ và tính sai số tái tạo. Đồng thời, bộ phân biệt sẽ cho rằng dữ liệu được đưa vào là bình thường hay bất thường. Kết quả từ cả hai, sau đó, được tổng hợp nhằm xác định xem dữ liệu có bất thường hay không.

#### **4.4.1.2. Các mô hình AE**

Các mô hình AE tái tạo dữ liệu bằng cách nén dữ liệu đầu vào nhiều chiều bằng bộ mã hoá (encoder) xuống không gian tiềm ẩn (latent space) với chiều không gian thấp hơn, để rồi từ đó tái tạo dữ liệu ban đầu thông qua bộ giải mã (decoder). Và nổi bật trong số các mô hình AE là Variational Auroencoder (VAE).

![VAE](https://arxiv.org/html/2503.13195v1/extracted/6287086/Figures/VAE.jpg)

VAE có cấu trúc bên trong giống với AE, tuy nhiên bộ mã hoá không chỉ nén dữ liệu xuống không gian tiềm ẩn mà còn học các phân phối xác suất của dữ liệu đầu vào như giá trị trung bình (mean) và phương sai (variance). Điều này giúp tạo ra một không gian tiềm ẩn chứa được nhiều ý nghĩa hơn. 

Để làm được điều đó, một thành phần quan trọng cần được giới thiệu trong VAE là độ phân kỳ Kullback-Leibler (KL), đo lường sự khác biệt giữa phân phối tiềm ẩn (latent distribution) được tạo ra bởi bộ mã hóa và một phân phối tiên nghiệm (prior distribution) được xác định trước (thường là phân bố chuẩn). Không giống như các AE truyền thống chỉ tập trung vào việc giảm thiểu lỗi tái tạo, VAE được huấn luyện bằng cách giảm thiểu sự kết hợp giữa lỗi tái tạo và độ phân kỳ KL.

Sự khác biệt này làm cho VAE mạnh mẽ hơn vì chúng không chỉ xem xét chất lượng tái tạo dữ liệu mà còn đảm bảo một không gian tiềm ẩn có cấu trúc thông qua độ phân kỳ KL. Bằng cách đó, độ phân kỳ KL giúp điều chỉnh không gian tiềm ẩn, đảm bảo rằng các biểu diễn được mã hóa được phân bố mượt mà và tập trung xung quanh phân bố tiên nghiệm. Việc điều chỉnh này làm giảm hiện tượng quá khớp, thúc đẩy khả năng khái quát hóa tốt hơn và giúp dễ dàng phân biệt giữa dữ liệu bình thường và bất thường, đặc biệt là trong các tập dữ liệu phức tạp và có chiều cao.

Bên cạnh đó, nhờ học được giá trị trung bình và phương sai của dữ liệu, sai số tái tạo không còn được tính bằng sự khác nhau giữa giá trị đầu vào và giá trị tái tạo nữa. Mà giờ đây là sự khác biệt giữa giá trị tái tạo và giá trị trung bình chia cho phương sai $error = \frac{|X' - \mu|}{\sigma^2}$.

#### **4.4.1.3. Các mô hình khuếch tán**

Mô hình khuếch tán là một loại mô hình tạo sinh hoạt động thông qua hai giai đoạn chính: một quá trình khuếch tán thuận cố định và một quá trình khuếch tán ngược có thể học được. Về mặt toán học, quá trình thuận bao gồm việc thêm dần nhiễu Gaussian vào dữ liệu $x_0$, biến đổi thành nhiễu $x_T$ sau $T$ bước.

Quá trình đảo ngược học cách khử nhiễu dữ liệu dần dần, loại bỏ nhiễu được thêm vào từng bước một. Mô hình học một phân phối tham số hóa $p_{\theta}(x_{t-1}|x_t)$ để đảo ngược quá trình thêm nhiễu, tái tạo dữ liệu gốc từ dữ liệu bị nhiễu.

Bằng cách loại bỏ nhiễu dần dần, các mô hình khuếch tán tạo ra các mẫu có độ chính xác cao, trước tiên thu thập các cấu trúc thô và sau đó tinh chỉnh các chi tiết trong mỗi bước. Các mô hình khuếch tán được huấn luyện trên dữ liệu bình thường để học phân bố dữ liệu cơ bản thông qua một quy trình loại bỏ nhiễu lặp đi lặp lại. Tương tự như các phương pháp dựa trên tái tạo khác, các bất thường có thể được xác định bằng cách đánh giá sai số tái tạo, trong đó sai số càng cao cho thấy dữ liệu càng lệch khỏi các mẫu bình thường đã học.

Các mô hình khuếch tán nổi bật so với GAN và VAE ở một số điểm chính. Chúng tránh được các vấn đề phổ biến như sự sụp đổ chế độ (collapse mode) trong GAN, nơi chỉ một tập hợp con của phân phối dữ liệu được thu thập, dẫn đến giảm tính đa dạng. Các mô hình khuếch tán cũng khắc phục được hiện tượng mờ nhòe liên quan đến VAE, thường là do mất mát dựa trên pixel (pixel-based loss) và do không gian tiềm ẩn của VAE nhỏ hơn. Bằng cách khử nhiễu dữ liệu lặp đi lặp lại, các mô hình khuếch tán duy trì cả độ chính xác cao và tính đa dạng trong đầu ra của chúng.
...
#### **4.4.2.1. Các mô hình RNN**

![RNN](https://arxiv.org/html/2503.13195v1/extracted/6287086/Figures/RNN.png)

Mạng nơ-ron truy hồi (RNN) là một loại mạng nơ-ron đặc biệt được thiết kế để xử lý dữ liệu tuần tự bằng cách nắm bắt các mối quan hệ phụ thuộc giữa các phần tử trong một chuỗi. Không giống như mạng nơ-ron tiêu chuẩn, RNN kết hợp một vectơ trạng thái trong tầng ẩn (hidden layer), cho phép chúng giữ lại thông tin từ các bước trước đó và mô hình hóa các mẫu tuần tự. Khả năng này làm cho chúng hiệu quả trong nhiều ứng dụng mà dữ liệu có thứ tự vốn có, chẳng hạn như nhật ký sự kiện, giám sát hệ thống và các chuỗi có cấu trúc trong an ninh mạng hoặc các quy trình công nghiệp.

Bằng cách duy trì trạng thái lặp lại, RNN có thể nắm bắt hiệu quả các mối quan hệ phụ thuộc giữa các bước khác nhau trong một chuỗi, khiến chúng rất phù hợp cho các tác vụ liên quan đến dữ liệu có thứ tự.

Tuy nhiên, RNN gặp phải vấn đề về sự bùng nổ hoặc suy biến gradient (exploding or vanishing gradients) khi xử lý các chuỗi dài. Mạng bộ nhớ dài hạn ngắn hạn (LSTM), một loại RNN chuyên biệt, đã được giới thiệu để giải quyết những vấn đề này. Cụ thể, LSTM thay thế lớp ẩn của RNN bằng một khối LSTM bao gồm các cổng đầu vào (input gate), cổng đầu ra (output gate) và cổng lãng quên (forget fate).

Hoặc ta cũng có thể sử dụng mô hình đơn vị truy hồi có cổng (GRU), là một phiên bản đơn giản hóa của LSTM chỉ bao gồm một cổng cập nhật và một cổng đặt lại và chỉ sử dụng trạng thái ẩn để biểu diễn cả thông tin ngắn hạn và dài hạn.

#### **4.4.2.2. Các sử dụng cơ chế chú ý**

![Attention](https://arxiv.org/html/2503.13195v1/extracted/6287086/Figures/Attention_for_AD.png)

Cơ chế chú ý ban đầu được áp dụng trong dịch máy, với ý tưởng cốt lõi là cho phép mạng nơ-ron tập trung vào các phần liên quan của giá trị đầu vào. Mặc dù các phương pháp dựa trên cơ chế chú ý đã cho thấy nhiều hứa hẹn trong phát hiện bất thường chuỗi thời gian, nhưng ứng dụng của chúng không chỉ giới hạn ở dữ liệu thời gian. Các phương pháp này có thể nắm bắt hiệu quả các mối quan hệ phụ thuộc trong nhiều loại dữ liệu khác nhau, bao gồm các tập dữ liệu không gian, không gian-thời gian và đa phương thức. Tính linh hoạt này mở rộng các trường hợp sử dụng của chúng trên các nhiệm vụ phát hiện bất thường khác nhau. So với các phương pháp dựa trên RNN, chúng phù hợp hơn cho các chuỗi dài hoặc phức tạp vì cơ chế chú ý có thể tính toán các mối quan hệ phụ thuộc giữa tất cả các vị trí trong chuỗi cùng một lúc, trong khi RNN xử lý các chuỗi theo trình tự, từng bước một.

#### **4.4.2.3. Các mô hình GNN**

![GNN](https://arxiv.org/html/2503.13195v1/extracted/6287086/Figures/GNN_for_TSAD.png)

Mạng nơ-ron đồ thị (GNN) ngày càng được chú ý trong các nhiệm vụ phát hiện bất thường, vì nhiều loại dữ liệu có thể được biểu diễn một cách tự nhiên dưới dạng cấu trúc đồ thị. Là các mô hình mạng nơ-ron được thiết kế đặc biệt để xử lý dữ liệu có cấu trúc đồ thị, GNN định nghĩa các nút, cạnh và đồ thị, trong đó các nút đại diện cho các phần tử riêng lẻ trong tập dữ liệu, chẳng hạn như các điểm dữ liệu trong một chuỗi, các giá trị đọc cảm biến trong dữ liệu đa biến hoặc các thực thể trong tập dữ liệu quan hệ.

GNN coi mỗi biến trong chuỗi thời gian đa biến như một nút để nắm bắt các mối quan hệ phức tạp giữa các chiều khác nhau. Mặc dù GNN có khả năng dự đoán là chính, nhưng chúng cũng hiệu quả trong việc phát hiện bất thường dựa trên tái tạo. Quyết định cuối cùng về việc chuỗi đầu vào có bất thường hay không chủ yếu dựa trên sai số dự đoán hoặc sự khác biệt về cấu trúc đồ thị, với sai số tái tạo đóng vai trò là thành phần bổ sung. 

Các phương pháp phát hiện bất thường dựa trên GNN vượt trội trong việc mô hình hóa các phụ thuộc phức tạp giữa các bước thời gian hoặc cảm biến, cung cấp tính linh hoạt để xử lý cả các mối quan hệ tĩnh và động trên các cấu trúc chuỗi thời gian đa dạng. Tuy nhiên, chúng vẫn phải đối mặt với những thách thức như độ phức tạp tính toán cao trên các đồ thị quy mô lớn và khó khăn trong việc xây dựng cấu hình cạnh và đồ thị tối ưu.

### **4.4.3. Cách tiếp cận dựa vào cả khả năng tái tạo và dự đoán của mô hình(Hybrid Method)**

Các phương pháp dựa trên khả năng tái tạo và dựa trên khả năng dự đoán của mô hình cung cấp các cách tiếp cận riêng biệt nhưng bổ sung cho nhau để xác định các bất thường. Cả hai phương pháp đều dựa vào sự khác biệt giữa đầu ra của mô hình và dữ liệu đầu vào thực tế như một chỉ báo về sự bất thường. Tuy nhiên, chúng khác nhau về cách xử lý dữ liệu và lĩnh vực ứng dụng. 

Các phương pháp dựa trên khả năng tái tạo của mô hình tập trung vào việc học phân bố cơ bản của dữ liệu bình thường. Sau khi được huấn luyện, mô hình cố gắng tái tạo dữ liệu đầu vào. Sai số tái tạo, được đo bằng sự khác biệt giữa dữ liệu gốc và dữ liệu được tái tạo, đóng vai trò là một chỉ báo quan trọng về các bất thường. Sai số tái tạo cao cho thấy dữ liệu lệch khỏi các mẫu bình thường mà mô hình đã học được. Cách tiếp cận này đặc biệt hiệu quả trong các trường hợp cần hiểu đầy đủ cấu trúc hoặc phân bố của dữ liệu, chẳng hạn như trong phát hiện bất thường dựa trên hình ảnh hoặc các tập dữ liệu đa chiều khác. 

Ngược lại, các phương pháp dựa trên khả năng dự đoán của mô hình tập trung vào việc dự báo các thuộc tính cụ thể hoặc các giá trị bị thiếu từ dữ liệu, thay vì tái tạo toàn bộ đầu vào. Các phương pháp này thường dự đoán các giá trị trong tương lai hoặc suy luận các điểm dữ liệu bị thiếu bằng cách tận dụng các đặc điểm đã biết. Nếu các giá trị được dự đoán khác biệt đáng kể so với các giá trị thực tế, điều này báo hiệu có một bất thường tiềm ẩn. Các phương pháp dựa trên dự đoán thường phù hợp hơn với các tập dữ liệu giàu đặc trưng, nơi việc dự đoán các biến cụ thể có thể giúp xác định các mẫu bất thường. Ví dụ, trong các ứng dụng như phát hiện gian lận, việc dự đoán hành vi hoặc giao dịch dự kiến có thể tiết lộ các bất thường khi kết quả dự đoán khác với kết quả quan sát được. 

Mặc dù cả hai phương pháp đều khác nhau về cách tiếp cận xử lý dữ liệu, nhưng chúng có thể bổ sung cho nhau rất tốt. Trong nhiều trường hợp, việc kết hợp cả hai cho phép phát hiện bất thường tốt hơn. Các mô hình tái tạo nắm bắt cấu trúc và thông tin tổng thể của dữ liệu, trong khi các mô hình dự đoán tập trung vào việc phát hiện sự sai lệch trong các biến hoặc đặc trưng cụ thể. Sự kết hợp này có thể cung cấp một giải pháp toàn diện hơn để xác định các bất thường trong các tập dữ liệu phức tạp trên nhiều lĩnh vực khác nhau. 

Ví dụ, để phát hiện bất thường bạn có thể vừa sử dụng kết quả từ các mô hình tái tạo, vừa sử dụng kết quả từ mô hình dự đoán, và vừa sử dụng kết quả khi cho cho dữ liệu nối tiếp qua mô hình dự đoán rồi tới mô hình tái tạo để tăng cường cho kết quả từ mô hình dự đoán trước đó.

## **4.5. Kết hợp giữa Học máy truyền thống và Học sâu**

Các phương pháp truyền thống và các phương pháp học sâu đều mang lại những lợi thế riêng biệt. Các phương pháp truyền thống, chẳng hạn như phân cụm (clustering) và vectơ hỗ trợ (SVM), thường đơn giản hơn, dễ hiểu hơn và hiệu quả về mặt tính toán. Những phương pháp này vượt trội trong việc cung cấp các quy trình ra quyết định minh bạch, khiến chúng phù hợp với các ứng dụng mà khả năng giải thích mô hình là rất quan trọng. Mặt khác, các phương pháp học sâu, với khả năng mô hình hóa các phân bố dữ liệu phức tạp, đa chiều, cung cấp độ chính xác và khả năng thích ứng phát hiện được nâng cao, đặc biệt là đối với các tập dữ liệu lớn và dữ liệu phi cấu trúc như hình ảnh và chuỗi.

Việc tích hợp các phương pháp truyền thống và học sâu nhằm mục đích tận dụng khả năng giải thích và tính đơn giản của các phương pháp truyền thống với tính mạnh mẽ và linh hoạt của các kỹ thuật học sâu. Bằng cách kết hợp các phương pháp này, các nhà nghiên cứu tìm cách tạo ra các mô hình lai duy trì độ chính xác đồng thời cung cấp những hiểu biết sâu sắc về quy trình ra quyết định cơ bản, cải thiện cả khả năng phát hiện và tính minh bạch của mô hình.

Với cách này, ta thường sử dụng mô hình học sâu để trích xuất các đặc trưng nhiều chiều, phức tạp trước rồi sau đó sử dụng các thuật toán như gom cụm hay vectơ hỗ trợ để có thể phân loại, phát hiện bất thường.
